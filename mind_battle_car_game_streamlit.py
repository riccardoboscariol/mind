...
if st.button("Avvia Generazione"):
    ...
    st.write(f"Entropia Auto Rossa: {entropy_1}")
    st.write(f"Entropia Auto Verde: {entropy_2}")
    ...
    st.write(f"Test Binomiale (cifre auto rossa): p-value = {binom_p_value_1:.4f}")

    binom_p_value_2 = binomtest(np.sum(random_bits_2), len(random_bits_2), alternative='two-sided').pvalue
    st.write(f"Test Binomiale (cifre auto verde): p-value = {binom_p_value_2:.4f}")

    st.write(f"Posizione Auto Rossa: {total_moves_1 / 100.0}")
    st.write(f"Posizione Auto Verde: {total_moves_2 / 100.0}")

    df = pd.DataFrame({
        "Auto Rossa": random_bits_1,
        "Auto Verde": random_bits_2
    })
